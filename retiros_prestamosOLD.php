...
function procesarRetiroHaberes($conn, $empleado) {
    ...
    try {
        ...
        $stmt->execute([$empleado['cod_empleado'], $anio_actual]);
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO historico_retiros_haberes 
                              (cod_empleado, cedula, nombre, monto_retirado, fecha_retiro, anio_retiro, estado)
...
function procesarSolicitudPrestamo($conn, $empleado) {
    ...
    if ($cod_fiador) {
        try {
            ...
            $stmt = $conn->prepare("SELECT COUNT(*) FROM historico_prestamos 
                                   WHERE cod_empleado = ? AND EXTRACT(YEAR FROM fecha_solicitud) = ?");
            $stmt->execute([$cod_fiador, $anio_actual]);
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO historico_prestamos 
                              (cod_empleado, cedula, nombre, monto_solicitado, monto_aprobado, 
                               ...
                               fecha_solicitud, estado, cod_fiador, nombre_fiador, saldo_pendiente)
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_DATE, 'Pendiente', ?, ?, ?)");
        $stmt->execute([
            $empleado['cod_empleado'],
            $empleado['cedula'],
...
function aprobarRetiro($conn) {
    ...
    try {
        ...
        // Actualizar estado del retiro - GUARDAR ROL
        $stmt = $conn->prepare("UPDATE historico_retiros_haberes 
                               SET estado = 'Aprobado', aprobado_por = ?, rol_aprobador = ?, fecha_retiro = CURRENT_DATE
                               WHERE id = ?");
...
<body>
    <div id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            <main class="main-content">
                    ...
                    <div class="associate-info-box" id="selectedAssociate">
                        ...
                        <div class="operation-selector">
                            ...
                            <div class="operation-buttons" id="operationButtons">
                                ...
                                <button type="button" class="btn btn-success" id="btnPrestamo">
                ...
                <!-- Formularios de Retiro y Préstamo (ocultos inicialmente) -->
                <div class="row">
                    <div class="col-md-12">
    ...
    <script>
            ...
            results.forEach(function(associate) {
                ...
                $item.html(`
                    <div class="d-flex justify-content-between">
                        <div>
                            <strong>${associate.nombre}</strong><br>
                            <small>Código: ${associate.cod_empleado} | Cédula: ${associate.cedula}</small>
                        </div>
                        <div class="text-end">
                            <small>Retiro ${retiroIcon} | Préstamo ${prestamoIcon}</small>
                        </div>
                    </div>
                    <div class="mt-2">
        ...
        function selectAssociate(associate) {
            ...
            // Mostrar botones de operación si tiene al menos una opción disponible
            const puedeRetirar = associate.puede_retirar.aprobado || false;
            const puedePrestamo = associate.puede_prestamo.aprobado || false;
            
            if (puedeRetirar || puedePrestamo) {
